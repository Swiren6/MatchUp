...
const FreelancerMatchPage = () => {
  ...
  const [application, setApplication] = useState({ cv: null, coverLetter: '' });
  ...
  const submitApplication = async () => {
    ...
    try {
      ...
      setApplication({ cv: null, coverLetter: '' });
    } catch {
      setNotification({ 
                  ...
                  <div>
                    ...
                    <label className="flex flex-col items-center justify-center border-2 border-dashed border-gray-300 rounded-lg p-6 cursor-pointer hover:border-blue-500 transition-colors">
                      ...
                      <input
                        type="file"
                        accept=".pdf"
                        onChange={(e) => setApplication({ ...application, cv: e.target.files[0] })}
                        className="hidden"
                      />
                    </label>
                  </div>
